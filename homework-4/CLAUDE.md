...
4. **Seeded defects** (documented in `context/bugs/001|002|003/bug-context.md`), chosen so the pipeline can realistically find and fix them in one run:
   ...
   - **Bug #4 (floating point) — NOT seeded; found by the pipeline.** `hasAtMostTwoDecimals()` in `src/validation.js` compares `Math.round(n*100) === n*100`, so valid amounts like `19.99`/`0.07`/`8.29` are rejected with `400`. The Research Verifier caught the Researcher's false "validation is correct" claim, which failed the quality gate and blocked run 1. Documented in `context/bugs/004/bug-context.md` and fixed on the re-run.
